...
export async function ciRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.post('/seed', async (request, reply) => {
    // Guard: CI_SECRET must be configured AND match request header
  ...
  fastify.post('/migrate-analytics', async (_request, reply) => {
    ...
    try {
      for (const q of queries) {
        ...
        results.push(`OK: ${q.slice(0, 60)}...`)
      }
      for (const q of indexes) {
        await prisma.$executeRawUnsafe(q)
        results.push(`IDX: ${q.slice(0, 60)}...`)
      }
      // Also create admin user if missing
      await prisma.$executeRawUnsafe(`
        INSERT INTO "User" (id, email, name, role, "passwordHash", "isActive", "isEmailVerified", "createdAt", "updatedAt")
        VALUES (gen_random_uuid(), 'admin@eclean.test', 'eClean Admin', 'ADMIN',
          '$2b$10$o6PEFfk4F8kUa7GS1qmIiemUNhKQyKxEcAVaEu4nxTn1dO8yRQMOy', true, true, NOW(), NOW())
        ON CONFLICT (email) DO UPDATE SET
          "passwordHash" = '$2b$10$o6PEFfk4F8kUa7GS1qmIiemUNhKQyKxEcAVaEu4nxTn1dO8yRQMOy',
          "isActive" = true, role = 'ADMIN'
      `)
      results.push('OK: Admin user created/updated')
      // Mark migration as applied
      await prisma.$executeRawUnsafe(`
        INSERT INTO "_prisma_migrations" (id, checksum, migration_name, finished_at, applied_steps_count)
        VALUES (gen_random_uuid(), 'manual_via_api', '20260328184537_add_analytics_intelligence_layer', NOW(), 1)
        ON CONFLICT DO NOTHING
      `)
      results.push('OK: Migration marked as applied')
    } catch (err: any) {
      results.push(`ERROR: ${err.message}`)
    }

    return reply.send({ ok: true, results, tablesCreated: 9 })
  })
}
