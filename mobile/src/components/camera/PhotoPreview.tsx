 ...
 *
 * Worker sees the full-screen photo and can:
 * - Retake: goes back to camera (discards this shot)
 * - Use Photo: confirms, photo saved to gallery + sent to onConfirm
 *
 * Shows GPS status and timestamp so worker knows metadata was captured.
 */
...
import type { PhotoType } from './CaptureCamera'
...
interface PhotoPreviewProps {
  ...
  saving:     boolean
  onRetake:   () => void
  onConfirm:  (uri: string, metadata: any) => void
}
...
export function PhotoPreview({ uri, photoType, saving, onRetake, onConfirm }: PhotoPreviewProps) {
  ...
  const handleConfirm = () => {
    onConfirm(uri, {
      ...
      lat:       null, // passed through from CaptureCamera
      lng:       null,
      deviceId:  'captured',
      taskId:    null,
    })
    ...
    <View style={s.container}>
      ...
      <View style={[s.metaRow, { top: insets.top + 64 }]}>
        ...
        <View style={s.metaChip}>
          ...
          <Text style={s.metaText}>GPS recorded</Text>
        </View>
      </View>
        ...
        >
          {saving
            ...
            : <>
                ...
                <Text style={s.confirmBtnText}>Use Photo</Text>
              </>
          }
        </TouchableOpacity>
      </View>

      {/* Evidence watermark band — simulates what will be on the upload */}
      <View style={[s.watermarkBand, { bottom: insets.bottom + 80 }]}>
        <Text style={s.watermarkText}>
          eClean  •  {now.toLocaleDateString('en-IN')} {now.toLocaleTimeString('en-IN')}  •  Evidence photo
        </Text>
      </View>
    </View>
  )
